...
# Which in turn was adapted from https://github.com/mahyarnajibi/FreeAdversarialTraining/blob/master/main_free.py
...
def parse_args():
    ...
    parser.add_argument('--clip', default=-1, type=int) # Clipping radius relative to epsilon (-1 means no clipping)
    parser.add_argument('--epsilon', default=4, type=int) # Epsilon perturbation radius
    parser.add_argument('--alpha', default=4, type=float) # Alpha FGSM step
    parser.add_argument('--unif', default=2, type=float) # Noise coefficient
...
def main():
    ...
    configs.alpha /= configs.DATA.max_color_value
    configs.epsilon /= configs.DATA.max_color_value
    
    # Create output folder
    if not os.path.isdir(os.path.join('/scratch/1/project/co_neurips/debug/fast_adv_models', configs.output_name)):
        os.makedirs(os.path.join('/scratch/1/project/co_neurips/debug/fast_adv_models', configs.output_name))
    ...
    for epoch in range(configs.TRAIN.start_epoch, configs.TRAIN.epochs):
        ...
        }, is_best, os.path.join('/scratch/1/project/co_neurips/debug/fast_adv_models', f'{configs.output_name}'),
        epoch + 1)
...
def train(train_loader, model, criterion, optimizer, epoch, lr_schedule, half=False): 
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if configs.TRAIN.random_init: 
            global_noise_data.uniform_(-configs.unif * configs.epsilon, configs.unif * configs.epsilon)
        for j in range(configs.ADV.n_repeats):
            ...
            pert = fgsm(grad, configs.alpha) # TODO: NFGSM
            global_noise_data[0:input.size(0)] += pert.data
            if configs.clip > 0:
                global_noise_data.clamp_(-configs.clip * configs.epsilon, configs.clip * configs.epsilon)
...
def get_input_grad(model, X, y, eps, criterion, delta_init='none', backprop=False):
    delta = torch.zeros_like(X).cuda()
    if 'random' in delta_init:
        delta.uniform_(-eps, eps)
        delta.clamp_(0, 1)
        if delta_init == 'random_corner':
